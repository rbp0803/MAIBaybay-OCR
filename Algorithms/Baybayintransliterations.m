...
% Modified dates for MAIBaybay version: September 2025 to April 2026                     % 
% ---------------------------------------------------------------------------------------%

%parent function: MAIBaybayOCR_App.mlapp
function [output2,transliterations2,single_lines3]=Baybayintransliterations(ModelUsed2,output,WBB,Baybayin_counter,Tagalog_words,A,RW)
tic;
    ...
    kk=zeros(1,length(transliterations));
       ...
       single_lines=[];
        for seq4=1:length(single_line)
          ...
          permn1=(1:kk1); permn2=permn_revised3(permn1,kk); [permn2_row, permn2_col]=size(permn2);      
...
single_lines3=single_lines2;                % The system takes all the combinations of transliterated Baybayin words
                                            % and are set to replace the Baybayin writings in the input image.

[line_row, ~]=size(single_lines3);
    dis1=['total phrases detected: ', num2str(size(single_lines3,1))];
    disp(dis1);
  if line_row>1
      single_lines3=contextfinder(single_lines3,output,RW);      % System will try to find the most sensible phrase with the aid of chatgpt-5 model
%     single_lines3=contextfinder_baselinemethod(single_lines3,output,RW);     % System will try to find the most sensible phrase with the aid of baseline LM
  else
